...
fn main() -> std::io::Result<()> {
    ...
    let dev = unsafe { SyncDevice::from_fd(fd)? }; // take the ownership of `fd`
    // let dev = unsafe { tun_rs::BorrowedSyncDevice::borrow_raw(fd)? }; // borrow fd without taking its ownership
    // let async_dev = unsafe { tun_rs::AsyncDevice::from_fd(fd)?}; // take the ownership of `fd`
    // let async_dev = unsafe { tun_rs::BorrowedAsyncDevice::borrow_raw(fd)?};  // borrow fd without taking its ownership
