...
macro_rules! impl_device {
    ($device:ty) => {
        ...
        impl $device {
            #[allow(dead_code)]
            pub fn new(device: SyncDevice) -> io::Result<$device> {
                <$device>::new_dev(device.0)
            }

            /// # Safety
            /// This method is safe if the provided fd is valid
            /// Construct a AsyncDevice from an existing file descriptor
            pub unsafe fn from_fd(fd: RawFd) -> io::Result<$device> {
                <$device>::new_dev(DeviceImpl::from_fd(fd)?)
            }
        ...
        impl $device {
            ...
            pub fn try_clone(&self) -> io::Result<Self> {
                <$device>::new_dev(self.get_ref().try_clone()?)
            }
...
#[cfg(feature = "async_tokio")]
pub use tokio::AsyncDevice as TokioDevice;
#[cfg(feature = "async_tokio")]
